...
def make_env(rank: int, base_seed: int = 20260822):
    def _init():
        env = FrozenPolicyOpponentEnv(
            agent_player=2,
            opponent_model_path=BLUE1_MODEL_PATH,
...
if __name__ == "__main__":
    for model_path in (RED1_MODEL_PATH, BLUE1_MODEL_PATH):
    ...
    try:
        # Loading Red1 restores its policy, optimizer, schedules, and counters.
        # Only Red is updated; Blue1 remains an environment-owned frozen model.
        ...
        if not optimizer_state:
            raise RuntimeError("Red1 optimizer state was not restored")

        print("=== Red2 continuation +10k vs stochastic frozen Blue1 ===", flush=True)

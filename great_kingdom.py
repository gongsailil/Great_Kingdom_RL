...
class GreatKingdomLogic:
    ...
    def copy(self):
        ...
        new_game.last_move_result = self.last_move_result
        return new_game
    ...
    def count_liberties(self, start_x, start_y, target_color):
        if self.board[start_y][start_x] != target_color:
            return 0
        group = set()
        liberties = set()
    ...
    def get_territory_owner(self, x, y):
        ...
        if is_red and not is_blue:
            return 2 if has_blue else 0
        return 0

    def is_impossible_action(self, x, y):
        """True only for actions that should be masked out.

        Suicide is intentionally NOT impossible: it is a selectable move
        that immediately loses the game.
        """
        if not self.is_on_board(x, y):
            return True
        if self.board[y][x] != 0:
            return True
        return self.get_territory_owner(x, y) != 0

    def get_playable_spots(self):
        """All selectable moves, including suicide moves."""
        return [
            (x, y)
            for y in range(BOARD_SIZE)
            for x in range(BOARD_SIZE)
            if not self.is_impossible_action(x, y)
        ]
    ...
    def place_stone_detailed(self, x, y):
        ...
        if not self.is_on_board(x, y) or self.board[y][x] != 0:
            self.last_move_result = MoveResult.IMPOSSIBLE_OCCUPIED
            return self.last_move_result

        if self.get_territory_owner(x, y) != 0:
            self.last_move_result = MoveResult.IMPOSSIBLE_TERRITORY
            return self.last_move_result
        ...
        # Capture has priority: if this move captures the opponent, it wins.
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            nx, ny = x + dx, y + dy
            if self.is_on_board(nx, ny) and self.board[ny][nx] == opp:
                if self.count_liberties(nx, ny, opp) == 0:
                    ...
                    self.last_move_result = MoveResult.CAPTURE_WIN
                    return self.last_move_result

        # Suicide is a legal/selectable move, but loses immediately.
        if self.count_liberties(x, y, curr) == 0:
            self.game_over = True
            self.winner = opp
            loser = "Blue" if curr == 1 else "Red"
            winner = "Blue" if opp == 1 else "Red"
            self.win_reason = f"{winner} Wins ({loser} Suicide)"
            self.last_move_result = MoveResult.SUICIDE_LOSS
            return self.last_move_result

        self.turn = opp
        self.last_move_result = MoveResult.NORMAL
        return self.last_move_result

    def place_stone(self, x, y):
        """Backward-compatible UI API.

        Returns False only when the action is impossible. A suicide move is
        accepted (True) and immediately ends the game as a loss.
        """
        result = self.place_stone_detailed(x, y)
    ...
    def check_game_end_simple(self):
        if self.game_over:
            return
        playable = self.get_playable_spots()
        if not playable:
            self.game_over = True
    ...
    def draw_board(self):
        ...
        for y in range(BOARD_SIZE):
            for x in range(BOARD_SIZE):
                ...
                if self.logic.board[y][x] != 0:
                    self.draw_piece(cx, cy, self.logic.board[y][x])
